	...
	getEmergenceKernel(function(kernel) {
		...
		mysql.available(function(error) {
			...
			mysql.directories = flatiron.common.directories.normalize({
				...
				,'#EMERGENCEDB': '#ROOT/data/emergence'
			});
		...
		flatiron.common.async.series({
			...
			,checkSocket: function(next) {
				fs.exists(mysql.socketPath, function(exists) {
					socketFileExists = exists;
					next();
				});
			}
			,checkPidFile: function(next) {
				mysql.getProcessId(function(error, pid) {
					if(!pid) {
						pidFilePid = false;
						pidFilePidRunning = false;
						return next();
					}
					...
					try {
						if(process.kill(pid, 'SIGHUP')) {
							pidFilePidRunning = true;
						}
					}
					catch(error) {
						if(error.code != 'ESRCH') {
							kernel.log.error('Encountered unknown error while testing PID:', error);
							return next('Unknown error while testing mysql PID: '+error.code);
						}
			...
			,searchRunningProccesses: function(next) {
				// skip if pid file found
				if(pidFilePidRunning) {
					return next();
				}
				
				child_process.exec('ps cax | grep -v grep | grep mysqld', function(error, stdout, stderr) {
					// grep will return an error code if it matched no lines
					runningProcsFound = !error;
					next();
				});
			}
			...
			if(socketFileExists && pidFilePidRunning) {
			...
			if(socketFileExists) {
				if(runningProcsFound) {
					kernel.log.error('An existing mysql socket was found and possible existing mysqld processes were detected but my pid file doesn\'t match one. To prevent accidental damage, nothing will be done. Terminate the existing processes or delete the existing socket file and try again to start the service.');
					return callback(new Error('mysqld has a socket and running processes but pid file is invalid'));
				}
				
				kernel.log.warn('mysql socket exists but no running processes were found, attempting to start up normally...');
			}
...
/**
 * Opens a new connection using emergence's root credentials
 */
mysql.connect = function(options, callback) {
	...
	getEmergenceKernel(function(kernel) {
		...
		mysql.start(function(error) {
			if(error) {
				return callback(error);
			}
	
			var connection = mysqlClient.createConnection(flatiron.common.mixin({
				socketPath: mysql.socketPath
				,user: kernel.config.get('services:mysql:username')
				,password: kernel.config.get('services:mysql:password')
			}, options));
...
mysql.getProcessId = function(callback) {
